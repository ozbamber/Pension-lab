(function (root) {
  ...
  function parseManagementFeesFromTokens(input) {
    ...
    const extract = (aliases) => {
      for (let index = 0; index < tokens.length; index += 1) {
        if (!containsAlias(tokens[index].text, aliases)) continue;
        const valueContext = tokens.slice(index, index + 6).map((token) => token.text).join(' ');
        const percentage = percentageFragments(valueContext)[0];
        const value = F.normalizeFinancialValue(percentage, { kind: 'rate' }).value;
        if (value != null && value <= 0.2) return { value, page: tokens[index].page };
      }
      return null;
    };
    return Object.fromEntries([
      ['depositManagementFeeRate', extract(ALIASES.depositFee)],
      ['balanceManagementFeeRate', extract(ALIASES.balanceFee)],
    ].filter(([, value]) => value));
  }
