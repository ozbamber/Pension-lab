...
- Progress is now page- and stage-aware, monotonic, visibly reaches 100%, exposes native `progressbar` ARIA values, and resets on completion, cancellation, and a new upload. Cold-start cancellation races runtime/worker initialization, returns the UI immediately, reports the cancelled state, and cleans a late worker; the deterministic browser regression completed that cancellation in **3ms**. The Chromium OCR suite increased from **5 to 6 groups** and observed every `aria-valuenow`/bar-width mutation.
...
- Two clean builds each generated **225 files**. The standalone SHA256 was **`50F5F9C36E377596F8CBE2A037EAA088CE925B3C1511BD4A8608B1ACEF2C2013`** and `dist/index.html` was **`960E135E0C4369B0DE96F67BBDD9A3969F1DE6CACAADCFCB8E143FFE604D0378`** both times. Source/dist SHA256 equality was also confirmed for `app.js` (`E845404A…`), `local-document-pipeline.js` (`94C97DF8…`), and `document-extraction.js` (`C3DCDA46…`).
...
- `npm run test:browser:ocr`: **6/6** native-report, local OCR, painted monotonic progress, privacy/session, mobile overflow, and warm/cold-start cancellation with late-worker cleanup passed.
...
- `npm run build`: generated **225 files** from canonical `app/`, including `dist/pension-lab-he-standalone.html`, `dist/_headers`, `dist/_redirects`, and `dist/404.html`; two consecutive builds produced identical standalone SHA256 `50F5F9C36E377596F8CBE2A037EAA088CE925B3C1511BD4A8608B1ACEF2C2013` and `dist/index.html` SHA256 `960E135E0C4369B0DE96F67BBDD9A3969F1DE6CACAADCFCB8E143FFE604D0378`.
