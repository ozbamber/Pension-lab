...
## 2026-08-31 OCR follow-up live release evidence

- Exact application commit `3e7618eb41251f6548b7b9f260af6836435fd351` was pushed to `origin/codex/pr2-interactive-simulator`. The unrelated root-level untracked Dataset v2 overlay remained outside staging, build input, and deployment; `origin/main` remained unchanged at `03870798ecccca31878a276fb99cd88dde5a417c`.
- The first follow-up Preview (`19a4f4b3-46cc-4bcf-92df-80c98ed318ca`, source `5394bf6`) was intentionally not promoted after the final review found that cancellation still waited for an active recognition call. The corrected commit added immediate active-recognition cancellation and repeated the complete release gate before a new Preview.
- Final immutable Preview deployment `a6382ee1-df8d-4032-be93-2d74e1e663b0` at `https://a6382ee1.pension-lab-5yh.pages.dev/` was verified at **2026-08-31T19:47:16.237Z**. Its branch alias is `https://codex-pr2-interactive-simula.pension-lab-5yh.pages.dev/`.
- Production deployment `d259ab72-1999-4c59-a7ed-9a16ff216743` at `https://d259ab72.pension-lab-5yh.pages.dev/` was verified at **2026-08-31T19:53:22.176Z**. The canonical site `https://pension-lab-5yh.pages.dev/` was independently verified at **2026-08-31T19:53:22.216Z**.
- Final Preview and Production used the exact same prebuilt 225-file `dist/` with no intervening rebuild. Deployed `index.html` SHA256 is `960E135E0C4369B0DE96F67BBDD9A3969F1DE6CACAADCFCB8E143FFE604D0378`; the standalone artifact is `6BB86000C36FE74BA9969C738B61F38EBF543573DF6078EBA9675545756985D0`; `app.js` is `E845404A20516F8E9BAB71DB8E3587AC0E1E9A0436AAA2A420A0BEAFE5E9DDA7`; `local-document-pipeline.js` is `57532B9EF31CBB6B9AE6619A56A0022F600E9578BF907CE136ADDDB47BA1659B`; and `document-extraction.js` is `C3DCDA46C151B13857586FA7834B139E9878C78C4F9C73D227277FC5ED153804`.
- Live verification confirmed the exact Hebrew and English language-model bytes (`D18B4DB5…` and `ED350F37…`), `Cache-Control: public, max-age=31536000, immutable` on both fingerprinted paths, 302 redirects from both legacy model paths, exact local/deployed application bytes, security headers, and a real 404 for a missing JavaScript asset.
- Headless Chromium loaded the canonical normal and `?demo=1` routes at **1440px** and **390px** with no positive horizontal overflow. Demo exposed all five controls. A scanned PDF then completed the live local-OCR path, reached the review step with two normalized months, made four same-origin OCR HTTP requests with no request body, and produced **0 external requests** and **0 browser errors**.
- Cloudflare records the Production source as `3e7618e` under its Pages `main` production label; this did not merge or push the Git repository's `main` branch. The immediate Production rollback target is `baeea5ab-faf0-4cd0-ae6e-725d5a894feb` (`3f5b469`).
