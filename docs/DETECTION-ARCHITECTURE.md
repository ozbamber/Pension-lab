...
4. Tokens are grouped into spatial rows. Every numeric observation receives a stable in-document candidate identity so one observation cannot silently fill multiple roles.
5. Candidate generation retains multiple amount and rate alternatives per anchor. Unary scores combine OCR confidence, format confidence, label proximity, page/row distance, and monthly-versus-annual semantics.
6. The payslip selector evaluates bounded combinations of salary and contribution candidates. It rewards amount/rate agreement and completeness, rejects reused observations and impossible ratios, and penalizes annual or YTD salary contexts when a monthly value is required.
...
9. Cross-document reconciliation compares independent observations with tolerance and chronology. Agreement raises confidence; disagreement remains visible and requires confirmation. When the payslip truly lacks a value, a directly observed report value may supply it with its report provenance intact.
...
Each semantic anchor retains only its highest-scoring bounded set of amount and rate candidates. The global evaluator explores the Cartesian product of those bounded sets and sorts deterministically by score and evidence completeness. This keeps browser runtime predictable while preserving genuine alternatives for adversarial layouts.

## Regression coverage

Synthetic tests cover misleading `925` neighbors, monthly versus annual salary, full contribution tuples, impossible relationships, ambiguous salaries, missing amounts or rates, amount/rate conflicts, conservative OCR corruption (`23,5OO`, `1,64S`, `1,64O`), annual/YTD report rows, fee-average confusion, cross-document agreement/conflict, and report-only supply of a genuinely missing payslip amount.
